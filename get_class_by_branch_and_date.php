...
if(!empty($courseID)) {
	$sql = mysqli_query($con, "SELECT * FROM class_activity_event WHERE DATE(eventStart) = '" . $date . "' AND branch = '" . $branchName . "' AND CourseID = '" . $courseID . "' AND active = 1");
}else{
	$sql = mysqli_query($con, "SELECT * FROM class_activity_event WHERE DATE(eventStart) = '" . $date . "' AND branch = '" . $branchName . "' AND active = 1");
}
